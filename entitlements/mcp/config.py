"""Environment-backed settings for the Entitlements MCP server.

Each field is read from the environment variable of the same name in upper
case (entitlements_api_url -> ENTITLEMENTS_API_URL), falling back to a .env file next
...
class Settings(BaseSettings):
    ...
    # This server is an HTTP client of the Entitlements API.
    entitlements_api_url: str = Field(
        default="http://127.0.0.1:8001",
        description="Base URL of the running API, for the MCP server to call",
    )
    entitlements_api_timeout: float = Field(
    ...
    mcp_port: int = Field(
        ...
        ge=1,
        le=65535,
        description="Port for the MCP server. Must differ from the API's 8001.",
    )

    @field_validator("entitlements_api_url")
    @classmethod
