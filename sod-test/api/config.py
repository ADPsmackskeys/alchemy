"""Environment-backed settings for the SoD Rules API.

Each field is read from the environment variable of the same name in upper
case (sod_rules_file -> SOD_RULES_FILE), falling back to a .env file next to
...
class Settings(BaseSettings):
    ...
    sod_rules_file: Path = Field(
        default=_HERE / "sod_rules.json",
    ...
    port: int = Field(default=8005, ge=1, le=65535, description="Port for `python main.py`")

    @field_validator("sod_rules_file")
    @classmethod
