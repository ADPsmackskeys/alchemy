...
run on separate hosts):
    ...
    claude mcp add --transport http peer-affinity http://127.0.0.1:8000/mcp
...
@mcp.tool(
    ...
    description=(
        ...
        "Each job_role belongs to exactly one department, so passing both job_roles "
        "and departments narrows nothing -- pass job_roles alone. "
        "If you want both a role-level and a department-level view, make the BROADER "
        "call only and group the rows yourself: the narrow result is always contained "
        "in the broad one, so asking narrow and then broad is a wasted call. "
