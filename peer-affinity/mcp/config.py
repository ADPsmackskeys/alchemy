"""Environment-backed settings for the Peer Affinity MCP server.

Each field is read from the environment variable of the same name in upper
case (peer_affinity_api_url -> PEER_AFFINITY_API_URL), falling back to a .env file next
...
class Settings(BaseSettings):
    ...
    # This server is an HTTP client of the Peer Affinity API.
    peer_affinity_api_url: str = Field(
        default="http://127.0.0.1:8003",
        description="Base URL of the running API, for the MCP server to call",
    )
    peer_affinity_api_timeout: float = Field(
    ...
    mcp_port: int = Field(
        ...
        ge=1,
        le=65535,
        description="Port for the MCP server. Must differ from the API's 8003.",
    )

    @field_validator("peer_affinity_api_url")
    @classmethod
