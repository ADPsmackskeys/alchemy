"""Environment-backed settings for the Requests MCP server.

Each field is read from the environment variable of the same name in upper
case (requests_api_url -> REQUESTS_API_URL), falling back to a .env file
next to this module, then to the default below. Values are validated once
at import time, so a typo like MCP_PORT=eight fails immediately with a
clear message instead of surfacing deep inside a tool call.
"""
...
class Settings(BaseSettings):
    ...
    # Used by main.py, which talks to the Requests API over HTTP.
    requests_api_url: str = Field(
        default="http://127.0.0.1:8000",
        description="Base URL of the running API, for the MCP server to call",
    )
    requests_api_timeout: float = Field(
    ...
    @field_validator("requests_api_url")
    @classmethod
