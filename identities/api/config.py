"""Environment-backed settings for the Identities API.

Each field is read from the environment variable of the same name in upper
case (identities_file -> IDENTITIES_FILE), falling back to a .env file next
...
class Settings(BaseSettings):
    ...
    identities_file: Path = Field(
        default=_HERE / "identities.json",
    ...
    port: int = Field(default=8002, ge=1, le=65535, description="Port for `python main.py`")

    @field_validator("identities_file")
    @classmethod
